...
def verifyForIndex(index):
	global rightCounter, totalIndexCounter, totalProfileCount
			...
			if os.path.isfile(errorOutputFilePath) and os.access(errorOutputFilePath, os.R_OK):
				...
				if errorOutput.find('crashed') != -1 and errorOutput.find('system') != -1:
					...
					if math.log(float(crashDistance), 10) >= latencyThreshold:
						rightCounter += 1
						# read profile count
						profileCountFile = open("./llfi.stat.prof.txt", 'r')
						line = profileCountFile.read()
						totalProfileCount = int(line.split("=")[1]) + totalProfileCount
						os.chdir("..")
						return
...
print "total profile count: " + `totalProfileCount`
